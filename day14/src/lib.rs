...
impl Polymer {
    ...
    pub fn char_occurencies(&self) -> HashMap<char, usize> {
    ...
    pub fn sorted_char_occurences(&self) -> Vec<(char, usize)> {
        let countmap = self.char_occurencies();
...
mod tests {
    ...
    fn polymer_step() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let polymer = Polymer::new("NNCB");

        let polymer1 = polymer.step(&rules);
        assert_eq!(polymer1, Polymer::new("NCNBCHB"));

        let polymer2 = polymer1.step(&rules);
        assert_eq!(polymer2, Polymer::new("NBCCNBBBCBHCB"));

        let polymer3 = polymer2.step(&rules);
        assert_eq!(polymer3, Polymer::new("NBBBCNCCNBBNBNBBCHBHHBCHB"));

        let polymer4 = polymer3.step(&rules);
        assert_eq!(
            polymer4,
            Polymer::new("NBBNBNBBCCNBCNCCNBBNBBNBBBNBBNBBCBHCBHHNHCBBCBHCB")
        );
    ...
    fn polymer_char_count() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut polymer = Polymer::new("NNCB");
